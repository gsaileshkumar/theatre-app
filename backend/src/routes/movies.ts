...
export default async function movieRoutes(app: FastifyInstance) {
  ...
  app.get('/showtime', {
    schema: { querystring: z.object({ id: z.coerce.number(), date: z.coerce.date() }) },
    handler: async (request, reply) => {
      try {
        const { id, date } = request.query as { id: number; date: Date };
        ...
        const rows = await app.db
          ...
          .where(and(eq(shows.movieId, id), eq(shows.currentStatus, sql`'ACTIVE'`), gt(shows.showTime, startOfDay), lt(shows.showTime, endOfDay)))
  ...
  app.post('/', {
    ...
    handler: async (request, reply) => {
      try {
        const { name, ticket_price } = request.body as z.infer<typeof movieBodySchema>;
  ...
  app.put('/', {
    schema: { body: movieBodySchema.extend({ id: z.number() }) },
    preHandler: (req, reply, done) => {
      if ((req as any).session?.user?.role !== 'ADMIN') { reply.code(401).send(RES_FAILURE); return; }
      done();
    },
    handler: async (request, reply) => {
      try {
        const { id, name, ticket_price } = request.body as z.infer<typeof movieBodySchema> & { id: number };
        const userId = (request as any).session.user!.id;
