...
export default async function bookingRoutes(app: FastifyInstance) {
  app.get('/show', async (request, reply) => {
    try {
      const { id } = z.object({ id: z.coerce.number() }).parse(request.query);
  ...
  app.post('/booktickets', async (request, reply) => {
    try {
      const { sequence_numbers, show_id } = bookTicketsSchema.parse(request.body);
  ...
  app.post('/checkin', async (request, reply) => {
    try {
      const { qr_data } = z.object({ qr_data: z.string() }).parse(request.body);
