...
export const GET_CURRENT_BOOKING_SUMMARY = `
    SELECT MOVIE_ID, MOVIE_NAME, SHOW_TIME, BOOKING_USER_ID, BOOKING_SHOW_ID, 
        STRING_AGG(BOOKING_SEQUENCE_NUMBER::VARCHAR, ', ') AS BOOKED_SEATS, true, false, $3, $4 FROM BOOKINGS
    INNER JOIN SHOWS
    ON BOOKING_SHOW_ID = SHOW_ID
    INNER JOIN MOVIES
    ON SHOW_MOVIE_ID = MOVIE_ID
    WHERE BOOKING_USER_ID = $1 AND BOOKING_SHOW_ID = $2
    GROUP BY BOOKING_SHOW_ID, BOOKING_USER_ID, MOVIE_ID, MOVIE_NAME, SHOW_TIME, BOOKING_UPDATED_AT
...
export const GET_BOOKING_SUMMARY_BY_ID = `
    SELECT 
    BOOKING_SUMMARY_ID AS BOOKING_ID,
    BOOKING_SUMMARY_MOVIE_ID AS MOVIE_ID,
    BOOKING_SUMMARY_MOVIE_NAME AS MOVIE_NAME,
    ...
    BOOKING_SUMMARY_CHECKED_IN AS CHECKED_IN
    FROM BOOKING_SUMMARY
    WHERE BOOKING_SUMMARY_USER_ID = $1 AND BOOKING_SUMMARY_ID = $2
`;

export const CONFIRM_CHECK_IN = `
    UPDATE BOOKING_SUMMARY 
    SET BOOKING_SUMMARY_CHECKED_IN = true, BOOKING_SUMMARY_UPDATED_AT = NOW(), BOOKING_SUMMARY_UPDATED_BY = $1
    WHERE BOOKING_SUMMARY_ID = $2
`;
